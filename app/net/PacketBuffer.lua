...
PacketBuffer.ENDIAN = clover.ByteArrayVarint.ENDIAN_BIG
...
function PacketBuffer.getBaseBA()
	return clover.ByteArrayVarint.new(PacketBuffer.ENDIAN)
end
...
function PacketBuffer:parsePackets(__byteString)
	...
	--local size = self._buf:readShort()
	--print("====================")
	--dump(size)
	while self._buf:getAvailable() >= __preLen do
				local __bodyLen = self._buf:readShort()
				...
				if __bodyLen <= PacketBuffer.PACKET_MAX_LEN then
					print("..................")
					__msgs[#__msgs+1] = self._buf:readString(__bodyLen)
				end
			--end
		--end
	end
	...
	if self._buf:getAvailable() <= 0 then
		...
		local __tmp = clover.ByteArrayVarint.new(PacketBuffer.ENDIAN)
